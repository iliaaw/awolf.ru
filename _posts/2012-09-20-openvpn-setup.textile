...
<ul>
  ...
  <li>Ключ Diffie Hellman. Нужен для установления защищенного соединения.</li>
</ul>
...
Ключ <code>ca.key</code> используется только для подписи сертификатов -- поэтому его можно не держать в <code>/etc/openvpn/</code>, но обязательно нужно сохранить в надежном месте и никому не показывать.
...
Android не поддерживает OpenVPN «из коробки». Поэтому находим в Google Play и ставим какой-нибудь OpenVPN клиент, права root-а для этого не обязательны. Вновь понадобится <code>ca.crt</code> и клиентский сертификат/ключ.
